...
This scraper extracts job listings from TEC Agency (BambooHR API) and imports them to peviitor.ro.

Target: https://wearetec.com/careers/
...
1. **Start with brand** - We know the brand (e.g., "TEC Agency")
...
8. **Scrape new jobs** - Extract jobs from BambooHR API
...
> **Important**: Scraper does NOT delete jobs from other sources (ANOFM, etc). It only upserts TEC Agency jobs. Existing jobs are preserved.
...
3. **Scrape jobs** - Extract jobs from BambooHR API
...
| `tests/validate-tec-agency-jobs.js` | CI fast validator (HEAD only); thin CLI over `src/job-validator.js` + `solr.js` |
...
| `tests/e2e/scraper.test.js` | End-to-end tests with real BambooHR API |
...
- **BambooHR Careers**: `https://tecss.bamboohr.com/careers/list`
